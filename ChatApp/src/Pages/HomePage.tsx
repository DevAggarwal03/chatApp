        ...
        <div className="flex w-full px-3 min-h-[90vh] gap-x-2">
                ...
                </div>
                {
                    contacts.map((contact:contactInterface) => (
                            <div key={contact.id} onClick={() => {setSelected(contact.id)}} className={`flex border-2 ${selected == contact.id ? 'border-white scale-103': 'border-none'} rounded-xl bg-[#023047] p-2 text-white gap-x-2`}>
                                <img src={contact.pic} className="size-[80px]"/>
